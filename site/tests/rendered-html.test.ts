...
Deno.test("renders the passed baseline and retained informational enterprise boundary", async () => {
  ...
  assertMatch(html, /One realm/);
  assertMatch(html, /Active baseline · Single-realm Railway/);
  assertMatch(html, /2,400/);
  assertMatch(html, /href="#proof">Explore the evidence/);
  assertMatch(html, /Supported planning/);
  assertMatch(html, /560 RPS/);
  assertMatch(html, /Retained candidate signal/);
  assertMatch(html, /1,680 RPS/);
  assertMatch(html, /5,600/);
  assertMatch(html, /16,800/);
  assertMatch(html, /173 ms p95/);
  assertMatch(html, /Size your realm/);
  assertMatch(html, /data-active-users="100000"/);
  assertMatch(html, /not Fleet proof/i);
  assertMatch(html, /The proof/);
  assertMatch(html, /Latency lenses/);
  assertMatch(html, /not additive/i);
  ...
  assertMatch(html, /different workload and much larger topology/i);
  assertEquals(html.match(/<h1\b/g)?.length, 1);
